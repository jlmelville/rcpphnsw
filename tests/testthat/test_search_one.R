...
expect_equal(res$idx, self_nn_index4[1, , drop = FALSE], tolerance =  1e-6)
expect_equal(res$dist, self_nn_dist4[1, , drop = FALSE], tolerance =  1e-6)
...
expect_equal(nbrs_with_distances$distance, self_nn_dist4[1, ], tolerance =  1e-6)
expect_error(index$getNNsList(ui10[1, ], 15, FALSE), "(?i)unable to find")

# Test deletion
index$markDeleted(1)
res <- hnsw_search(ui10[1, , drop = FALSE], index, k = 4)
expect_equal(res$idx[1, ], c(6, 10, 3, 7), tolerance =  1e-6)
expect_equal(res$dist[1, ], c(self_nn_dist4[1, 2:4], 0.812404), tolerance =  1e-6)
