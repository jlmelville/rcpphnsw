...
test_that("add/get matrix", {
  ...
  expect_equal(items, self_nn_index4, ignore_attr = TRUE)
  ...
  expect_error(
    ann2$getAllNNsListCol(ui10t, k = 15, FALSE),
    "(?i)unable to find"
  )
  res <- ann2$getAllNNsListCol(ui10t, k = 4, include_distances = TRUE)
  expect_equal(t(res$item), self_nn_index4, ignore_attr = TRUE)
  expect_equal(
    sqrt(t(res$distance)),
    self_nn_dist4,
    ignore_attr = TRUE,
  ...
  expect_error(
    ann2$getAllNNsListCol(ui10, k = 4, include_distances = TRUE),
    "incorrect dimensions"
  )
  ...
  expect_equal(t(items), self_nn_index4, ignore_attr = TRUE)
})
