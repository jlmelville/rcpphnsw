...
* Invalid inputs are now rejected before native conversion. Search `k` cannot exceed the number
  of undeleted items. Coordinates must fit in single precision, and cosine vectors must have a
  nonzero norm. Very large vector magnitudes are rejected to prevent distance overflow; this
  also applies to existing checkpoints, including their deleted items.
* Fixed data races in parallel construction and search. If insertion fails after modifying an
  index, the index becomes unusable and must be rebuilt or reloaded.
* Loading rejects incompatible dimensions and malformed checkpoints. Save failures are reported
  to R, including failures to flush or close the file.
* Empty indexes retain their saved capacity when loaded without an explicit capacity. As before,
  a supplied capacity below the stored item count falls back to the saved value.
* Resizing an empty index to zero is rejected safely. A failed native resize makes the index
  unusable; discard it and rebuild or reload. Fixed a memory leak during resize failure.
* `grain_size` now applies to all threaded insertion and search operations.
* Updated hnswlib to [version 0.9.0](https://github.com/nmslib/hnswlib/releases/tag/v0.9.0).
